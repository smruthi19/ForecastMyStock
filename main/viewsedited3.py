...
# Select country
    ...
    def NormalModel():
        ...
        # url=('https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=VZ&apikey=XCIMZL4S81DCVOQO')
    ...
    def LogModel():
        ...
        today=datetime.date.today()
        today1=today.strftime('%Y-%m-%d')
        # url=('https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=NYSE:VZ&apikey=XCIMZL4S81DCVOQO')
        query=request.GET.get('name')
        print(query)
        result = str(query)
        print(result)
    ...
    # plt.plot(xlist,new_list, color='blue')
    data_tuples = list(zip(xlist[15:],new_list[15:]))
    df1=pd.DataFrame(data_tuples, columns=['Date','Value'])
    print(df1)
    print("data")
    #define cutoff date
    ...
    # p=plt.plot(df1[df1.Date >= cutoff].Date, df1[df1.Date >= cutoff].Value, c = "r", label = "after {}".format(cutoff))
    #
    ...
    df2 = DataFrame(xlist[16:22],columns=['Dates'])
    # print (df)
    df2['values']=new_list[16:22]
